...
cow_mount=$(grep -c "modified_content" "$MOUNT_DIR/base.txt" 2>/dev/null); cow_mount=${cow_mount:-0}
cow_upper=$(grep -c "modified_content" "$UPPER_DIR/base.txt" 2>/dev/null); cow_upper=${cow_upper:-0}
cow_lower=$(grep -c "modified_content" "$LOWER_DIR/base.txt" 2>/dev/null); cow_lower=${cow_lower:-0}
